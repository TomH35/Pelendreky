  ...
  methods: {
    ...
    async fetchArticle() {
      ...
      try {
        ...
        // Increment view count after successfully loading the article
        await this.incrementViewCount(article_slug);
      } catch (error) {
    ...
    async incrementViewCount(article_slug) {
      try {
        await fetch(`/Backend/article_api/ArticleIncrementViewCountAPI.php`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
          body: JSON.stringify({ article_slug })
        });
      } catch (error) {
